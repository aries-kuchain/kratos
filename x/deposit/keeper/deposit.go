...
func  (k Keeper)  GetMortgageRatio(ctx sdk.Context,depositID string) (err error,baseRatio,currentRatio sdk.Int) {
	depositInfo, found := k.GetDepositInfo(ctx, depositID)
	if !found {
	...
	err,baseRatio,currentRatio = k.singerKeeper.GetMortgageRatio(ctx,depositID,quoteAmount)
	return err,baseRatio,currentRatio
}